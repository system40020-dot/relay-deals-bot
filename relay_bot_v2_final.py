...
def is_blocked_html(html_content):
    """Check karta hai ki page reCAPTCHA/access-denied/block-page hai ya real content."""
    if not html_content or len(html_content) < 1000:
        markers = ["recaptcha", "are you a human", "access denied", "site maintenance", "captcha"]
        lower = (html_content or "").lower()
        return any(m in lower for m in markers) or len(html_content or "") < 500
    return False

def fetch_html_via_browser(url, referer=None):
    """Real Patchright browser se ek page ka HTML fetch karta hai - JS run hoti hai, real
    cookies/TLS-fingerprint hoti hai, isliye plain requests se zyaada resilient hai reCAPTCHA
    jaise JS-based challenges ke against. Sirf FALLBACK ke roop mein use hota hai (heavy hai)."""
    browser = None
    try:
        ...
        print(f"DEBUG CATEGORY: Patchright-fallback fetch failed: {e}")
        return None, None
    finally:
...
def fetch_html_via_camoufox(url, referer=None):
    """Camoufox (custom anti-detect Firefox) se page fetch karta hai. Patchright se bhi zyada
    stealth hai (fingerprint-spoofing C++/engine-level pe hoti hai, JS-shim se pakadna mushkil
    hai), lekin sabse heavy/dheema tier hai. Isliye TEESRE aur AAKHRI fallback ke roop mein hi
    use hota hai - jab plain request aur Patchright dono fail/blocked ho chuke hon."""
    try:
        ...
        print(f"DEBUG CATEGORY: Camoufox-fallback fetch failed: {e}")
        return None, None


def fetch_html_tiered(url, referer=None, session_holder_key="flipkart", homepage_url=None):
    """3-tier fetch system - koi bhi scraper (single-product ho ya category ho, kisi bhi
    platform ka ho) ise use kar sakta hai:
      Tier 1: Plain warmed request  -> tez, kam RAM, zyaadatar cases mein kaafi hai
      Tier 2: Patchright (Chromium) -> agar Tier 1 block ho jaaye
      Tier 3: Camoufox (Firefox)    -> agar Tier 2 bhi block ho jaaye, sabse stealthy

    Timeouts jaan-bujhkar chhote rakhe gaye hain (12s / 22s / 20s) taaki teeno tier mil ke bhi
    total ~55s ke andar rahein - warna ek hi tier ka 40s timeout poora budget kha jaata aur
    baaki tiers ko mauka hi nahi milta.

    Return: (html_content, final_url, tier_used) - tier_used sirf debug/logging ke liye hai.
    """
    homepage_url = homepage_url or referer or url

    # ---- Tier 1: Plain request ----
    try:
        session = get_warmed_session(homepage_url, session_holder_key)
        headers = {"Referer": referer} if referer else {}
        resp = session.get(url, headers=headers, timeout=12, allow_redirects=True)
        html_content = resp.text
        final_url = resp.url
        if not is_blocked_html(html_content):
            print(f"DEBUG TIERED: Tier 1 (plain request) SUCCEEDED for {url[:60]}")
            return html_content, final_url, "plain_request"
        print(f"DEBUG TIERED: Tier 1 blocked for {url[:60]}, trying Tier 2 (Patchright)...")
    except Exception as e:
        print(f"DEBUG TIERED: Tier 1 failed for {url[:60]} -> {e}, trying Tier 2 (Patchright)...")

    # ---- Tier 2: Patchright ----
    html_content, final_url = fetch_html_via_browser(url, referer=referer)
    if html_content and not is_blocked_html(html_content):
        print(f"DEBUG TIERED: Tier 2 (Patchright) SUCCEEDED for {url[:60]}")
        return html_content, final_url, "patchright"
    print(f"DEBUG TIERED: Tier 2 blocked/failed for {url[:60]}, trying Tier 3 (Camoufox)...")

    # ---- Tier 3: Camoufox (last resort, sabse heavy) ----
    html_content, final_url = fetch_html_via_camoufox(url, referer=referer)
    if html_content and not is_blocked_html(html_content):
        print(f"DEBUG TIERED: Tier 3 (Camoufox) SUCCEEDED for {url[:60]}")
        return html_content, final_url, "camoufox"

    print(f"DEBUG TIERED: All 3 tiers failed/blocked for {url[:60]}")
    return html_content, final_url, "all_failed"

def fetch_category_products_flipkart(category_url, max_products=10, max_pages=4, only_page=None):
    """Flipkart category/listing page se multiple products nikaalta hai (bina Playwright ke).
    only_page diya ho to SIRF wahi ek page fetch hoti hai (sequential-page scraping ke liye).
    Ek persistent session (cookies ke saath) use karta hai taaki real-browser jaisa lage."""
    products = []
    seen_pids = set()
    try:
        time.sleep(random.uniform(0.5, 1.5))
        html_content, base_url, tier_used = fetch_html_tiered(
            category_url, referer="https://www.flipkart.com/",
            session_holder_key="flipkart", homepage_url="https://www.flipkart.com/"
        )
        if not html_content:
            print(f"DEBUG CATEGORY: Flipkart fetch completely failed (all 3 tiers) for {category_url[:60]}")
            return products

        print(f"DEBUG CATEGORY: Final URL = {base_url} (via {tier_used})")
        ...
        if js_redirect:
            ...
            time.sleep(random.uniform(0.5, 1.5))
            html_content, base_url, tier_used = fetch_html_tiered(
                real_url, referer="https://www.flipkart.com/",
                session_holder_key="flipkart", homepage_url="https://www.flipkart.com/"
            )
            if not html_content:
                print(f"DEBUG CATEGORY: Flipkart fetch (after JS redirect) completely failed (all 3 tiers)")
                return products
            print(f"DEBUG CATEGORY: Final URL (after JS redirect) = {base_url} (via {tier_used})")
        ...
        for page_num in ([only_page] if only_page else range(1, max_pages + 1)):
            if page_num == 1:
                ...
                try:
                    time.sleep(random.uniform(1, 2.5))
                    page_html, _, page_tier = fetch_html_tiered(
                        page_url, referer=base_url,
                        session_holder_key="flipkart", homepage_url="https://www.flipkart.com/"
                    )
                    if not page_html:
                        print(f"DEBUG CATEGORY: Page {page_num} fetch failed (all 3 tiers)")
                        break
                except Exception as e:
                    print(f"DEBUG CATEGORY: Page {page_num} fetch failed: {e}")
                    break
                print(f"DEBUG CATEGORY: Page {page_num} HTML length = {len(page_html)} (via {page_tier})")
        ...
        return products
def fetch_category_products_amazon(category_url, max_products=10, max_pages=3, only_page=None):
    ...
    try:
        time.sleep(random.uniform(0.5, 1.5))
        html_content, base_url, tier_used = fetch_html_tiered(
            category_url, referer="https://www.amazon.in/",
            session_holder_key="amazon", homepage_url="https://www.amazon.in/"
        )
        if not html_content:
            print(f"DEBUG CATEGORY (Amazon): Fetch completely failed (all 3 tiers) for {category_url[:60]}")
            return products

        print(f"DEBUG CATEGORY (Amazon): Final URL = {base_url} (via {tier_used})")
        ...
        if js_redirect:
            ...
            time.sleep(random.uniform(0.5, 1.5))
            html_content, base_url, tier_used = fetch_html_tiered(
                real_url, referer="https://www.amazon.in/",
                session_holder_key="amazon", homepage_url="https://www.amazon.in/"
            )
            if not html_content:
                print(f"DEBUG CATEGORY (Amazon): Fetch (after JS redirect) completely failed (all 3 tiers)")
                return products
            print(f"DEBUG CATEGORY (Amazon): HTML length (after redirect) = {len(html_content)} (via {tier_used})")
        ...
        for page_num in ([only_page] if only_page else range(1, max_pages + 1)):
            if page_num == 1:
                ...
                try:
                    ...
                    if not page_html:
                        print(f"DEBUG CATEGORY (Amazon): Page {page_num} fetch failed (all 3 tiers)")
                        break
                except Exception as e:
                    print(f"DEBUG CATEGORY (Amazon): Page {page_num} fetch failed: {e}")
                    break
                print(f"DEBUG CATEGORY (Amazon): Page {page_num} HTML length = {len(page_html)} (via {page_tier})")
                if len(page_html) < 3000:
                    print(f"DEBUG CATEGORY (Amazon): Page {page_num} looks empty/blocked, stopping")
                    break

            # Order-agnostic: sirf valid data-asin dhoondo, chahe data-component-type
            # pehle ho ya baad mein (Amazon ka exact attribute-order badalta rehta hai)
            card_blocks = re.findall(r'data-asin="([A-Z0-9]{10})"', page_html)
            print(f"DEBUG CATEGORY (Amazon): Page {page_num} raw data-asin matches = {len(card_blocks)}")
            found_this_page = 0
            ...
            for asin in card_blocks:
                ...
                title, price, mrp, rating, image_url = None, None, None, None, None

                # Amazon ek hi ASIN ko kai jagah reference karta hai (ads/widgets/tracking-scripts
                # mein bhi, na sirf asli product-card mein). Sirf PEHLA occurrence lena galat nikla
                # (52 ASINs mile the, 0 products bane) - isliye HAR occurrence try karte hain jab
                # tak title+price dono na mil jaayein.
                for m in re.finditer(re.escape(f'data-asin="{asin}"'), page_html):
                    window = page_html[m.start(): m.start() + 5000]

                    t_match = re.search(r'<h2[^>]*>.*?<span[^>]*>([^<]{10,150})</span>', window, re.DOTALL)
                    if not t_match:
                        t_match = re.search(r'<img[^>]*alt="([^"]{10,150})"', window)

                    p_match = re.search(r'class="a-price-whole"[^>]*>([\d,]+)', window)
...
def fetch_category_products(category_url, max_products=10, only_page=None):
    ...
    if "amazon" in url_lower or "amzn" in url_lower:
        return fetch_category_products_amazon(category_url, max_products, only_page=only_page)
    ...
    return fetch_category_products_flipkart(category_url, max_products, only_page=only_page)
...
# ================= GITHUB GIST (permanent storage - Render redeploy se survive karta hai) =================
GITHUB_TOKEN = os.getenv("GITHUB_TOKEN", "")
GIST_ID = os.getenv("GIST_ID", "")
PERSISTED_FILES = ["category_posted.json", "category_stats.json", "price_tracking.json",
                    "category_page_cursor.json", "bot_settings.json", "active_categories.json"]

def gist_headers():
    return {"Authorization": f"token {GITHUB_TOKEN}", "Accept": "application/vnd.github+json"}

def sync_file_to_gist(filename):
    """Ek local JSON file ko GitHub Gist pe push karta hai - taaki Render redeploy se data safe rahe."""
    ...
    try:
        ...
        print(f"DEBUG GIST: sync-to-gist failed for {filename}: {e}")

def restore_files_from_gist():
    """Startup pe Gist se saari saved files wapas local disk pe la deta hai, taaki
    redeploy ke baad bhi settings/history yaad rahein."""
    if not GITHUB_TOKEN or not GIST_ID:
        print("DEBUG GIST: GITHUB_TOKEN/GIST_ID set nahi hai, local-only mode mein chal raha hai (data redeploy pe reset hoga)")
        return
    try:
        resp = requests.get(f"https://api.github.com/gists/{GIST_ID}", headers=gist_headers(), timeout=15)
        if not resp.ok:
            print(f"DEBUG GIST: restore failed, status {resp.status_code} - {resp.text[:200]}")
            return
        files = resp.json().get("files", {})
        restored = 0
        for fname in PERSISTED_FILES:
            if fname in files and files[fname].get("content"):
                with open(fname, "w") as f:
                    f.write(files[fname]["content"])
                restored += 1
        print(f"DEBUG GIST: {restored} files restore hui Gist se")
    except Exception as e:
        print(f"DEBUG GIST: restore error: {e}")
...
def parse_product_html(html_content, final_url):
    """Playwright/Patchright/Camoufox se mile html_content ko parse karke title/price/image/mrp/
    discount nikalta hai. fetch_product_metadata_with_playwright aur fetch_product_metadata_with_camoufox
    dono isi shared function ko use karte hain (JSON-LD, MRP-patterns, discount-validation - sab yahan hai)."""
...
def fetch_product_metadata_with_playwright(url):
    """Patchright (Chromium) real headless browser se product page fetch karta hai, phir
    shared parse_product_html() se title/price/image/mrp/discount nikalta hai."""
    browser = None
    try:
        with sync_playwright() as p:
            browser = p.chromium.launch(
            ...
            return parse_product_html(html_content, final_url)
    except Exception as e:
...
def fetch_product_metadata_with_camoufox(url):
    """Camoufox (anti-detect Firefox) se product page fetch karta hai - TEESRA aur AAKHRI tier,
    jab lightweight fetch aur Patchright dono fail/incomplete ho chuke hon. Reuses fetch_html_via_camoufox
    (Part 1 mein bana tha) aur wahi shared parse_product_html() se parse karta hai."""
    html_content, final_url = fetch_html_via_camoufox(url)
    if not html_content:
        return None
    print(f"DEBUG CAMOUFOX: Final URL = {final_url}")
    print(f"DEBUG CAMOUFOX: HTML length = {len(html_content)}")
    return parse_product_html(html_content, final_url or url)
...
def extract_title_from_url_slug(url):
    """Jab scraping poori tarah fail ho (jaise Myntra/Ajio ka bot-block), lekin URL redirect
    follow ho chuka ho (expanded/final link mil chuka ho), to us URL ke path-slug se hi
    ek usable title nikaalne ki koshish karta hai. Product ka Buy Now link isse touch nahi
    hota - sirf title-generation ke liye use hota hai."""
    if not url:
        return None
    try:
        ...
        generic_words = {"buy", "p", "pr", "dp", "s", "product", "products", "shirts",
                          "shoes", "mailers", "gp", "itm"}
        candidates = []
        for seg in segments:
            low = seg.lower()
            ...
            if re.fullmatch(r'\d+', seg):
                continue
            # ID-jaisa lagne wala segment (bina hyphen/plus ke, bas letters+numbers mila hua) skip karo
            if re.fullmatch(r'[a-zA-Z0-9]{6,}', seg) and '-' not in seg and '+' not in seg:
                continue
        ...
        # Sabse zyaada hyphen/plus wala segment usually poora product-naam wala slug hota hai
        best = max(candidates, key=lambda s: s.count('-') + s.count('+'))
        title = best.replace('-', ' ').replace('+', ' ').strip()
...
def format_caption(title, emoji, link, scraped, show_price_line=True):
    ...
    return "\n".join(lines)
def post_deal(image_url, caption, ph_link, chat_id=None):
    target_chat = chat_id or MAIN_CHAT_ID
    ...
    if image_url:
        try:
            ...
            payload = {"chat_id": target_chat, "caption": caption, "parse_mode": "HTML", "reply_markup": keyboard}
    ...
    payload = {"chat_id": target_chat, "text": caption, "parse_mode": "HTML", "disable_web_page_preview": True, "reply_markup": keyboard}
...
def save_posted_pids(pids):
    ...
    try:
        ...
        sync_file_to_gist(CATEGORY_STATE_FILE)
    except Exception as e:
...
def update_category_stats(url, extracted_delta=0, posted_delta=0, queue_remaining=None, title=None):
    """Har category ka extracted/posted count aur queue-mein-baaki count track karta hai."""
    with _stats_lock:
        try:
            try:
            ...
            entry = data.get(url, {"extracted_total": 0, "posted_total": 0, "queue_remaining": 0,
                                    "last_posted_title": None, "last_run": None})
            entry["extracted_total"] += extracted_delta
            entry["posted_total"] += posted_delta
            if queue_remaining is not None:
                entry["queue_remaining"] = queue_remaining
            if title:
                entry["last_posted_title"] = title
            entry["last_run"] = time.time()
            data[url] = entry

            with open(CATEGORY_STATS_FILE, "w") as f:
                json.dump(data, f)
            sync_file_to_gist(CATEGORY_STATS_FILE)
        except Exception as e:
            print(f"DEBUG STATS: Could not update stats: {e}")
...
# ================= PAGE-CURSOR (sequential page-by-page scraping, no auto-loop) =================
PAGE_CURSOR_FILE = "category_page_cursor.json"
MAX_PAGES_PER_CATEGORY = 5

def get_current_page(category_url):
    try:
        with open(PAGE_CURSOR_FILE, "r") as f:
            data = json.load(f)
        return data.get(category_url, {}).get("page", 1)
    except:
        return 1

def advance_page(category_url):
    """Agla page number set karta hai. Agar MAX_PAGES_PER_CATEGORY cross ho jaaye,
    LOOP BACK NAHI karta (jaisa aapne kaha, product-repeat ka risk hai) - iske bajaye
    'exhausted' return karta hai taaki category pause ho jaaye aur owner ko alert mile."""
    try:
        try:
            with open(PAGE_CURSOR_FILE, "r") as f:
                data = json.load(f)
        ...
        current = data.get(category_url, {}).get("page", 1)
        next_page = current + 1

        if next_page > MAX_PAGES_PER_CATEGORY:
            data[category_url] = {"page": current, "exhausted": True}
            with open(PAGE_CURSOR_FILE, "w") as f:
                json.dump(data, f)
            sync_file_to_gist(PAGE_CURSOR_FILE)
            return None  # exhausted signal

        data[category_url] = {"page": next_page, "exhausted": False}
        with open(PAGE_CURSOR_FILE, "w") as f:
            json.dump(data, f)
        sync_file_to_gist(PAGE_CURSOR_FILE)
        return next_page
    except Exception as e:
        print(f"DEBUG PAGE-CURSOR: Could not advance: {e}")
        return None

def is_category_exhausted(category_url):
    try:
        with open(PAGE_CURSOR_FILE, "r") as f:
            data = json.load(f)
        return data.get(category_url, {}).get("exhausted", False)
    except:
        return False

def reset_page_cursor(category_url):
    try:
        with open(PAGE_CURSOR_FILE, "r") as f:
            data = json.load(f)
    except:
        data = {}
    data[category_url] = {"page": 1, "exhausted": False}
    try:
        with open(PAGE_CURSOR_FILE, "w") as f:
            json.dump(data, f)
        sync_file_to_gist(PAGE_CURSOR_FILE)
    except Exception as e:
        print(f"DEBUG PAGE-CURSOR: Could not reset: {e}")

# ================= PRICE-DROP TRACKING =================
PRICE_TRACK_FILE = "price_tracking.json"
PRICE_TRACK_EXPIRY_DAYS = 10

def track_price(link, title, price, image_url, chat_id=None):
    """Ek posted deal ka price save karta hai, taaki baad mein price-drop check ho sake."""
    ...
    try:
        ...
        data[link] = {
            ...
            "chat_id": chat_id, "first_seen": data.get(link, {}).get("first_seen", time.time()),
            "last_checked": time.time()
        }

        cutoff = time.time() - (PRICE_TRACK_EXPIRY_DAYS * 86400)
        data = {k: v for k, v in data.items() if v.get("first_seen", 0) > cutoff}

        with open(PRICE_TRACK_FILE, "w") as f:
            json.dump(data, f)
        sync_file_to_gist(PRICE_TRACK_FILE)
    except Exception as e:
        print(f"DEBUG PRICE-TRACK: Could not save: {e}")
...
def check_price_drops():
    ...
    if not data:
        print("DEBUG PRICE-DROP: Koi tracked deal nahi hai abhi")
        return

    print(f"DEBUG PRICE-DROP: {len(data)} tracked deals check kar rahe hain...")
    changed = False
    for link, entry in list(data.items()):
        try:
            time.sleep(random.uniform(1, 2))
            scraped = fetch_product_metadata_lightweight(link)
            if not scraped or not scraped.get("price"):
                scraped = fetch_product_metadata_with_playwright(link)

            new_price = scraped.get("price") if scraped else None
            if not new_price:
                continue
            ...
            if new_price < old_price:
                ...
                print(f"DEBUG PRICE-DROP: Price dropped for '{entry['title'][:40]}' - ₹{old_price} -> ₹{new_price}")
                caption = (
                    f"🔻 <b>PRICE DROPPED AGAIN!</b> 🔻\n\n"
                    f"👀 🛍️ {entry['title']}\n\n"
                    f"💰 Price: <s>₹{old_price:,.0f}</s> ₹{new_price:,.0f} ({drop_pct}% further off)\n\n"
                    f"🛒 BUY NOW: {link}\n\n"
                    f"📢 JOIN FOR MORE DEALS: {CHANNEL_HANDLE}"
                )
                ph_link = build_price_history_link(get_canonical_url(link), entry["title"])
                post_deal(entry.get("image_url"), caption, ph_link, chat_id=entry.get("chat_id"))
            ...
            print(f"DEBUG PRICE-DROP error for {link[:40]}: {e}")
    ...
    try:
        ...
        print(f"DEBUG PRICE-DROP: Could not save after check: {e}")

    print(f"DEBUG PRICE-DROP: Check complete, {len(data)} still tracked")

def start_price_drop_checker():
    """Background thread - har BOT_SETTINGS['price_check_hours'] ghante mein saare tracked
    deals ka price dobara check karta hai."""
    def loop():
        time.sleep(300)  # startup ke turant baad nahi, thoda ruk ke
        while True:
            try:
                check_price_drops()
            except Exception as e:
                print(f"DEBUG PRICE-DROP loop error: {e}")
            hours = BOT_SETTINGS.get("price_check_hours", 4)
            time.sleep(max(1, hours) * 3600)
    threading.Thread(target=loop, daemon=True).start()
    print("DEBUG PRICE-DROP: Checker thread started")

def is_within_active_hours():
    """IST (UTC+5:30) ke hisaab se check karta hai ki abhi posting-allowed hours hain ya nahi."""
    start_h = BOT_SETTINGS.get("active_hours_start")
    end_h = BOT_SETTINGS.get("active_hours_end")
    if start_h is None or end_h is None:
        return True  # koi restriction set nahi hai
    ist_now = datetime.utcnow() + timedelta(hours=5, minutes=30)
    hour = ist_now.hour
    if start_h <= end_h:
        return start_h <= hour < end_h
    else:
        # jaise 22 se 6 baje tak (raat ko cross karta hua range)
...
BOT_SETTINGS = {
    "paused": False,
    "interval_minutes": 30,       # backward-compat (jab min/max range nahi diya gaya ho)
    "interval_min": 20,           # dynamic interval range - anti-bot detection se bachne ke liye
    "interval_max": 40,
    "min_discount": 0,
    "min_rating": 0.0,
    "price_min": 0.0,
    ...
    "removed_categories": set(),  # jo category PERMANENTLY delete ho chuki hai
    "active_hours_start": None,   # None = hamesha active. Jaise 9 (9 AM IST)
    "active_hours_end": None,     # Jaise 23 (11 PM IST)
    "category_channels": {},      # {category_url: chat_id} - multi-channel posting
    "price_check_hours": 4,       # price-drop check kitni baar chale
}

BOT_SETTINGS_FILE = "bot_settings.json"

def save_bot_settings():
    """BOT_SETTINGS ko disk pe aur Gist pe save karta hai (set -> list convert karke JSON-safe banata hai)."""
    try:
        serializable = dict(BOT_SETTINGS)
        serializable["muted_categories"] = list(BOT_SETTINGS["muted_categories"])
        serializable["removed_categories"] = list(BOT_SETTINGS["removed_categories"])
        with open(BOT_SETTINGS_FILE, "w") as f:
            json.dump(serializable, f)
        sync_file_to_gist(BOT_SETTINGS_FILE)
    except Exception as e:
        print(f"DEBUG SETTINGS: Could not save: {e}")

def load_bot_settings():
    """Startup pe saved settings wapas load karta hai (Gist se restore hone ke baad)."""
    try:
        with open(BOT_SETTINGS_FILE, "r") as f:
            saved = json.load(f)
        for k, v in saved.items():
            if k in ("muted_categories", "removed_categories"):
                BOT_SETTINGS[k] = set(v)
            else:
                BOT_SETTINGS[k] = v
        print("DEBUG SETTINGS: Purani settings load ho gayi")
    except Exception as e:
        print(f"DEBUG SETTINGS: Koi saved settings nahi mili, defaults use ho rahe hain ({e})")

_category_failure_count = {}  # {url: consecutive failure count} - error-alert ke liye
FAILURE_ALERT_THRESHOLD = 3

ACTIVE_CATEGORIES_FILE = "active_categories.json"

def save_active_categories():
    """Jo bhi categories abhi active hain (env-var se ya /addcategory / /search se), unki list
    save karta hai - taaki redeploy ke baad dynamically-added categories bhi wapas start ho sakein."""
    try:
        with open(ACTIVE_CATEGORIES_FILE, "w") as f:
            json.dump({"urls": list(_category_scheduler_state["running_urls"])}, f)
        sync_file_to_gist(ACTIVE_CATEGORIES_FILE)
    except Exception as e:
        print(f"DEBUG SCHEDULER: Could not save active categories: {e}")

def load_active_categories():
    try:
        with open(ACTIVE_CATEGORIES_FILE, "r") as f:
...
def start_category_scheduler(category_url, interval_minutes=30, min_discount=0, min_rating=0,
    ...
    def loop():
        ...
        while True:
            try:
                if category_url in BOT_SETTINGS.get("removed_categories", set()):
                    print(f"DEBUG SCHEDULER [{category_url[:40]}]: Removed, stopping thread permanently")
                    _category_scheduler_state["running_urls"].discard(category_url)
                    save_active_categories()
                    break
                if BOT_SETTINGS["paused"] or category_url in BOT_SETTINGS["muted_categories"]:
                ...
                elif is_category_exhausted(category_url):
                    print(f"DEBUG SCHEDULER [{category_url[:40]}]: Exhausted (max {MAX_PAGES_PER_CATEGORY} pages done), skipping. Use /resetcategory to restart.")
                elif not is_within_active_hours():
                    print(f"DEBUG SCHEDULER [{category_url[:40]}]: Outside active hours, skipping this cycle")
                else:
                    posted_pids = load_posted_pids()

                    if not queue:
                        current_page = get_current_page(category_url)
                        print(f"DEBUG SCHEDULER [{category_url[:40]}]: Queue empty, fetching page {current_page}...")
                        raw = fetch_category_products(category_url, max_products=max_products, only_page=current_page)
                        ...
                        print(f"DEBUG SCHEDULER [{category_url[:40]}]: Page {current_page} -> {len(raw)} scraped, {len(filtered)} new after filters")
                        queue = filtered
                        update_category_stats(category_url, extracted_delta=len(raw), queue_remaining=len(queue))

                        if len(filtered) == 0:
                            # Is page se ab kuch naya nahi mil raha - agle page pe badho (loop-back NAHI karenge)
                            next_page = advance_page(category_url)
                            if next_page is None:
                                print(f"DEBUG SCHEDULER [{category_url[:40]}]: All {MAX_PAGES_PER_CATEGORY} pages exhausted, pausing this category.")
                                if OWNER_ID:
                                    send_owner_message(
                                        f"✅ <b>Category poori scan ho gayi:</b>\n{category_url[:70]}\n\n"
                                        f"Saare {MAX_PAGES_PER_CATEGORY} pages ke products post ho chuke hain. "
                                        f"Yeh category ab pause hai (dobara repeat posts nahi karega).\n\n"
                                        f"Naye products ke liye /resetcategory {category_url[:40]} bhejo (page 1 se dobara shuru hoga)."
                                    )
                            else:
                                print(f"DEBUG SCHEDULER [{category_url[:40]}]: Moving to page {next_page} next cycle")

                        if len(raw) == 0:
                            _category_failure_count[category_url] = _category_failure_count.get(category_url, 0) + 1
                            if _category_failure_count[category_url] == FAILURE_ALERT_THRESHOLD and OWNER_ID:
                                send_owner_message(
                                    f"⚠️ <b>Warning:</b> Category is failing repeatedly (0 products, {FAILURE_ALERT_THRESHOLD} baar):\n{category_url[:70]}\n\n"
                                    f"Ho sakta hai block/reCAPTCHA ho gaya ho. /mute {category_url[:40]} kar sakte ho."
                                )
                        else:
                            _category_failure_count[category_url] = 0
                    ...
                    if queue:
                        ...
                        target_channel = BOT_SETTINGS["category_channels"].get(category_url)
                        caption = format_caption(title, emoji, buy_link, scraped)
                        post_deal(product.get("image_url"), caption, ph_link, chat_id=target_channel)
                        print(f"DEBUG SCHEDULER [{category_url[:40]}]: Posted -> {title[:50]}")

                        posted_pids.add(product.get("_pid"))
                        save_posted_pids(posted_pids)
                        update_category_stats(category_url, posted_delta=1, queue_remaining=len(queue), title=title)
                        track_price(buy_link, title, product["price"], product.get("image_url"), chat_id=target_channel)
                    else:
                        print(f"DEBUG SCHEDULER [{category_url[:40]}]: Nothing new to post this cycle")
                        update_category_stats(category_url, queue_remaining=0)

            except Exception as e:
                print(f"DEBUG SCHEDULER error [{category_url[:40]}]: {e}")
                _category_failure_count[category_url] = _category_failure_count.get(category_url, 0) + 1
                if _category_failure_count[category_url] == FAILURE_ALERT_THRESHOLD and OWNER_ID:
                    send_owner_message(f"⚠️ <b>Error:</b> Category scheduler bar-bar crash ho raha hai:\n{category_url[:70]}\n\nError: {str(e)[:150]}")

            # Dynamic interval - fixed jitter ki jagah poora min-max range se random pick karta hai,
            # taaki posting-pattern predictable na lage anti-bot systems ke liye
            lo = BOT_SETTINGS.get("interval_min", 20)
            hi = BOT_SETTINGS.get("interval_max", 40)
            if lo > hi:
                lo, hi = hi, lo
            sleep_min = max(3, random.uniform(lo, hi))
            time.sleep(sleep_min * 60)

    threading.Thread(target=loop, daemon=True).start()
    print(f"DEBUG SCHEDULER: Started for {category_url[:60]} (dynamic interval, staggered by {start_delay}s)")

def start_all_category_schedulers():
    """CATEGORY_URLS env var (permanent list) + persisted active-categories (jo /addcategory ya
    /search se dynamically add hui thi, Gist se restore hui) - dono ko milaake sab start karta hai."""
    urls_raw = os.getenv("CATEGORY_URLS", "")
    env_urls = [u.strip() for u in urls_raw.split(",") if u.strip()]
    persisted_urls = load_active_categories()

    # Dono lists milao, duplicates hatao (order preserve karte hue)
    seen = set()
    urls = []
    removed = BOT_SETTINGS.get("removed_categories", set())
    for u in env_urls + persisted_urls:
        if u not in seen and u not in removed:
            seen.add(u)
            urls.append(u)

    if not urls:
        print("DEBUG SCHEDULER: Koi category nahi mili (CATEGORY_URLS env var aur persisted list dono khaali hain)")
        return
    ...
    print(f"DEBUG SCHEDULER: {len(urls)} categories auto-started ({len(env_urls)} env-var se, {len(persisted_urls)} persisted-history se)")
...
# ================= WORKFLOW =================
def _pick_better_scrape(a, b):
    """Do scraped results (dict ya None) mein se zyaada complete wala chunta hai - taaki agar
    ek tier partial success de (jaise sirf title mila) aur agla tier poori tarah fail ho jaaye,
    toh pehla wala result waste na ho."""
    def score(s):
        if not s:
            return -1
        sc = 0
...
def process_message(msg):
    ...
    if not scraped or not scraped.get("price") or not scraped.get("image_url"):
        scraped = _pick_better_scrape(scraped, fetch_product_metadata_with_playwright(aff_link))
    if not scraped or not scraped.get("price") or not scraped.get("image_url"):
        scraped = _pick_better_scrape(scraped, fetch_product_metadata_with_camoufox(aff_link))
    ...
    if not title or title == "SPECIAL OFFER DEAL":
        # Pehle URL-slug se try karo (kaam aata hai jab site block kare lekin redirect resolve ho gaya ho)
        slug_title = extract_title_from_url_slug(final_link) if final_link else None
        if slug_title:
            title = slug_title
        else:
    ...
    post_deal(image_url, caption, ph_link)
    if scraped and scraped.get("price"):
        track_price(aff_link, title, scraped["price"], image_url)
...
def handle_owner_command(text, chat_id):
    ...
    if cmd in ("/start", "/help"):
        send_owner_message(
            ...
            "/setinterval (min) (max) — jaise /setinterval 20 40 (dynamic range, anti-bot ke liye)\n"
            ...
            "/removecategory (partial-url) — category PERMANENTLY delete karo\n"
            "/clearmuted — saari muted categories ek saath clear karo\n"
            "/listcategories — saari active categories dikhao\n"
            "/categorystats — har category ka extracted/posted/remaining count\n"
            "/setactivehours (start) (end) — jaise /setactivehours 9 23 (IST, 24hr format)\n"
            "/clearactivehours — active-hours restriction hata do (24x7 posting)\n"
            "/setchannel (partial-url) (chat_id) — us category ke deals alag channel pe bhejo\n"
            "/setpricecheck (hours) — price-drop check kitni baar chale (default 4)\n"
            "/pricetracking — kitne deals price-drop ke liye tracked hain\n"
            "/resetcategory (partial-url) — category ka page-cursor reset karo (page 1 se dobara)\n"
            "/search (flipkart|amazon) (keyword) | (discount) (rating) (pmin) (pmax) — keyword-based scraping+posting shuru karo",
        ...
        hours = (f"{BOT_SETTINGS['active_hours_start']}:00 - {BOT_SETTINGS['active_hours_end']}:00 IST"
                 if BOT_SETTINGS["active_hours_start"] is not None else "24x7 (no restriction)")
        send_owner_message(
            f"<b>📊 Current Status</b>\n\n"
            f"Paused: <b>{'Yes' if BOT_SETTINGS['paused'] else 'No'}</b>\n"
            f"Interval: <b>{BOT_SETTINGS['interval_min']}-{BOT_SETTINGS['interval_max']} min (dynamic)</b>\n"
            f"Active Hours: <b>{hours}</b>\n"
            ...
            f"Price-check every: <b>{BOT_SETTINGS['price_check_hours']}h</b>\n"
            f"Muted: {muted}\n\n"
        ...
        try:
            if len(parts) >= 3:
                ...
                BOT_SETTINGS["interval_min"] = lo
                BOT_SETTINGS["interval_max"] = hi
                BOT_SETTINGS["interval_minutes"] = (lo + hi) // 2
                send_owner_message(f"✅ Interval ab {lo}-{hi} minute (random) set ho gaya.")
            else:
                val = int(parts[1])
                BOT_SETTINGS["interval_min"] = max(3, val - 5)
                BOT_SETTINGS["interval_max"] = val + 5
                BOT_SETTINGS["interval_minutes"] = val
                send_owner_message(f"✅ Interval ab ~{val} minute (±5 range) set ho gaya.")
        except:
            send_owner_message("❌ Usage: /setinterval 20 40  (ya sirf /setinterval 30 for ±5 range)")
    elif cmd == "/setfilters":
        try:
    ...
    elif cmd == "/removecategory":
        if len(parts) < 2:
            send_owner_message("❌ Usage: /removecategory (partial-url)")
        else:
            match = next((u for u in _category_scheduler_state["running_urls"] if parts[1] in u), None)
            if match:
                BOT_SETTINGS["removed_categories"].add(match)
                BOT_SETTINGS["muted_categories"].discard(match)
                BOT_SETTINGS["category_channels"].pop(match, None)
                send_owner_message(
                    f"🗑️ Permanently removed: {match[:60]}\n"
                    f"(Agar yeh CATEGORY_URLS env var mein bhi hai, toh wahan se hataana zaroori hai "
                    f"warna list clean rahegi lekin next redeploy pe thread turant khud-ba-khud ruk jaayega.)"
                )
            else:
                send_owner_message("❌ Yeh URL kisi active category se match nahi hua.")
    elif cmd == "/clearmuted":
        count = len(BOT_SETTINGS["muted_categories"])
        BOT_SETTINGS["muted_categories"].clear()
        send_owner_message(f"🧹 {count} muted categories clear kar diye.")
    elif cmd == "/listcategories":
        cats = "\n".join(f"• {u[:60]}" for u in _category_scheduler_state["running_urls"]) or "Koi category active nahi"
        send_owner_message(f"<b>📋 Active Categories:</b>\n{cats}")
    elif cmd == "/categorystats":
        stats = load_category_stats()
        if not stats:
            send_owner_message("📊 Abhi tak koi stats record nahi hua.")
        else:
            lines = []
            for url, s in stats.items():
                lines.append(
                    f"• {url[:50]}\n"
                    f"   Extracted: {s.get('extracted_total', 0)} | Posted: {s.get('posted_total', 0)} | Remaining in queue: {s.get('queue_remaining', 0)}"
                )
            send_owner_message(f"<b>📊 Category-wise Stats</b>\n\n" + "\n\n".join(lines))
    elif cmd == "/setactivehours":
        try:
            BOT_SETTINGS["active_hours_start"] = int(parts[1])
            BOT_SETTINGS["active_hours_end"] = int(parts[2])
            send_owner_message(f"✅ Active hours set: {parts[1]}:00 - {parts[2]}:00 IST. Iske bahar posting rukegi.")
        except:
            send_owner_message("❌ Usage: /setactivehours 9 23  (24hr format, IST)")
    elif cmd == "/clearactivehours":
        BOT_SETTINGS["active_hours_start"] = None
        BOT_SETTINGS["active_hours_end"] = None
        send_owner_message("✅ Active-hours restriction hata di gayi, ab 24x7 posting hogi.")
    elif cmd == "/setchannel":
        if len(parts) < 3:
            send_owner_message("❌ Usage: /setchannel (partial-url) (chat_id)")
        else:
            match = next((u for u in _category_scheduler_state["running_urls"] if parts[1] in u), None)
            if match:
                BOT_SETTINGS["category_channels"][match] = parts[2]
                send_owner_message(f"✅ {match[:50]} ab {parts[2]} pe post hoga.")
            else:
                send_owner_message("❌ Yeh URL kisi active category se match nahi hua.")
    elif cmd == "/setpricecheck":
        try:
            BOT_SETTINGS["price_check_hours"] = int(parts[1])
            send_owner_message(f"✅ Price-drop check ab har {parts[1]} ghante mein hoga.")
        except:
            send_owner_message("❌ Usage: /setpricecheck 4")
    elif cmd == "/pricetracking":
        try:
            with open(PRICE_TRACK_FILE, "r") as f:
                data = json.load(f)
            send_owner_message(f"📉 <b>{len(data)}</b> deals currently price-tracked ho rahe hain.")
        except:
            send_owner_message("📉 Abhi koi deal tracked nahi hai.")
    elif cmd == "/resetcategory":
        if len(parts) < 2:
            send_owner_message("❌ Usage: /resetcategory (partial-url)")
        else:
            match = next((u for u in _category_scheduler_state["running_urls"] if parts[1] in u), None)
            if match:
                reset_page_cursor(match)
                send_owner_message(f"🔄 Reset ho gaya: {match[:60]}\nAb page 1 se dobara scan hoga (already-posted items skip honge, purane 15-din wale expire honge apne aap).")
            else:
                send_owner_message("❌ Yeh URL kisi active category se match nahi hua.")
    elif cmd == "/search":
        # Format: /search flipkart running shoes | 20 3.5 300 3000
        raw_text = text.strip()[len(cmd):].strip()
        if "|" in raw_text:
            head, filt = raw_text.split("|", 1)
        else:
            head, filt = raw_text, ""
        head_parts = head.strip().split()
        if len(head_parts) < 2:
            send_owner_message(
                "❌ Usage: /search (flipkart|amazon) (keyword) | (discount) (rating) (price_min) (price_max)\n"
                "Jaise: /search flipkart running shoes | 20 3.5 300 3000\n"
                "(filters optional hain - default settings use hongi)"
            )
        else:
            platform = head_parts[0].lower()
            keyword = " ".join(head_parts[1:])
            filt_parts = filt.strip().split()
            try:
                discount = int(filt_parts[0]) if len(filt_parts) > 0 else BOT_SETTINGS["min_discount"]
                rating = float(filt_parts[1]) if len(filt_parts) > 1 else BOT_SETTINGS["min_rating"]
                pmin = float(filt_parts[2]) if len(filt_parts) > 2 else BOT_SETTINGS["price_min"]
                pmax = float(filt_parts[3]) if len(filt_parts) > 3 and filt_parts[3] != "0" else BOT_SETTINGS["price_max"]
            except:
                send_owner_message("❌ Filters samajh nahi aaye. Format: (discount) (rating) (price_min) (price_max)")
                return

            if platform == "flipkart":
                search_url = f"https://www.flipkart.com/search?q={urllib.parse.quote(keyword)}"
            elif platform == "amazon":
                search_url = f"https://www.amazon.in/s?k={urllib.parse.quote(keyword)}"
            else:
                send_owner_message("❌ Platform sirf 'flipkart' ya 'amazon' ho sakta hai.")
                return

            start_category_scheduler(search_url, BOT_SETTINGS["interval_minutes"], discount, rating, pmin, pmax)
            send_owner_message(
                f"🔍 Search-based scraping shuru: <b>{keyword}</b> ({platform})\n"
                f"Filters: {discount}%+ off, {rating}+★, ₹{pmin}-₹{pmax or '∞'}\n\n"
                f"Yeh bilkul category ki tarah kaam karega - naya deal milega toh automatically post hoga, "
                f"affiliate-tag khud add ho jaayega."
            )
    else:
...
if __name__ == "__main__":
    restore_files_from_gist()
    load_bot_settings()
    threading.Thread(target=run_web, daemon=True).start()
    start_all_category_schedulers()
    start_price_drop_checker()
    background_bot_loop()
