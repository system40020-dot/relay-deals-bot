...
def fetch_product_metadata(url):
    try:
        ...
        if og_title: title = html.unescape(og_title.group(1))
...
def background_bot_loop():
    """Continuous background loop to check for new messages without waiting for web pings."""
    ...
    while True:
        ...
        time.sleep(3) # Chhota gap taaki API overload na ho

if __name__ == "__main__":
    # Start Flask web server in a separate thread (for Render health check)
    threading.Thread(target=run_web, daemon=True).start()
    
    # Start Telegram bot listener in the main thread (or background loop)
    background_bot_loop()
