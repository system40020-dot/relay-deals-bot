...
# Poore bot mein kabhi bhi EK SAATH sirf ek hi browser (Patchright ya Camoufox) chale - chahe
# category-thread se ho, price-drop-checker se ho, ya single-product message se ho. Isse
# multiple browsers ek saath launch hoke CPU/RAM crash hone se bachte hain (sab sequentially,
# ek-ek karke apna turn lete hain).
...
def fetch_html_via_camoufox(url, referer=None):
    """Camoufox (anti-detect Firefox) se page fetch karta hai. Patchright se bhi zyada stealth hai.
    SIRF Myntra/Ajio/Meesho ke liye use hota hai - Amazon/Flipkart ka Patchright-flow isse
    bilkul alag hai. Global lock ke andar chalta hai taaki Patchright ke saath ek saath na chale."""
    with _browser_lock:
        try:
            ...
            print(f"DEBUG CAMOUFOX: fetch failed for {url[:60]} -> {e}")
            return None, None
...
MYNTRA_AJIO_MEESHO_DOMAINS = ("myntra.com", "ajio.com", "meesho.com")

def is_myntra_ajio_meesho(url):
    """URL Myntra/Ajio/Meesho me se kisi ek ka hai kya - inhi teeno ke liye Camoufox
    (extra-stealth) tier use hota hai, Amazon/Flipkart ke liye nahi."""
    if not url:
        return False
    u = url.lower()
    return any(d in u for d in MYNTRA_AJIO_MEESHO_DOMAINS)


def fetch_product_metadata_with_camoufox(url):
    """Camoufox (anti-detect Firefox) se product page fetch karta hai - SIRF Myntra/Ajio/Meesho
    ke liye use hota hai. Parsing-logic jaan-bujhkar fetch_product_metadata_with_playwright se
    DUPLICATE rakhi hai (shared function banane se bachne ke liye), taaki Amazon/Flipkart ka
    function kabhi galti se bhi is refactor mein touch na ho."""
    html_content, final_url = fetch_html_via_camoufox(url)
    if not html_content:
        print("DEBUG CAMOUFOX: fetch returned no content")
        return None
    ...
    if len(html_content) < 1000:
        print(f"DEBUG CAMOUFOX: SHORT HTML CONTENT = {html_content}")
...
def _pick_better_scrape(a, b):
    """Do scraped results (dict ya None) mein se zyaada complete wala chunta hai."""
    def score(s):
        if not s:
            return -1
        sc = 0
...
def process_message(msg):
    ...
    # Amazon/Flipkart ke liye bilkul original, stable 2-tier chain - isse bilkul chheda nahi gaya
    ...
    # Sirf Myntra/Ajio/Meesho ke liye - agar abhi bhi incomplete hai, Camoufox extra-tier try karo
    resolved_link = scraped.get("link") if scraped else scrape_url
    if is_myntra_ajio_meesho(resolved_link) and (not scraped or not scraped.get("price") or not scraped.get("image_url")):
        scraped = _pick_better_scrape(scraped, fetch_product_metadata_with_camoufox(scrape_url))
