...
# User-Agent Rotation pool to bypass anti-bot / scraping blocks
USER_AGENTS = [
...
def fetch_product_metadata(url):
    try:
        ...
        # Advanced Title Parsing (checking og:title first, then fallback to HTML <title>)
        ...
        if og_image: image_url = og_image.group(1)
...
def get_emoji(text):
    ...
    if any(k in t for k in ["shoe", "sneaker"]): return "👟"
    if any(k in t for k in ["watch", "smartwatch"]): return "⌚"
    if any(k in t for k in ["headphone", "audio"]): return "🎧"
    return "🛍️"

def format_caption(title, coupon, emoji, link, scraped):
    ...
    if coupon:
        lines.append(f"🏷️ <b>{coupon.upper()}</b>\n")

    lines.extend([f"🛒 <b>BUY NOW:</b> {link}\n", f"📢 <b>JOIN FOR MORE DEALS:</b> {CHANNEL_HANDLE}"])
    return "\n".join(lines)

def get_tg_photo(file_id):
    try:
        r = requests.get(f"https://api.telegram.org/bot{RELAY_BOT_TOKEN}/getFile", params={"file_id": file_id}, headers=get_random_headers(), timeout=8)
        if r.ok:
...
def process_message(msg):
    ...
    aff_link, explicit_ph = None, None
    for u in urls:
        if "pricehistory" in u.lower(): explicit_ph = u
        elif not aff_link: aff_link = u
            
    if not aff_link: return
    ...
    title = re.sub(r'(?i)(\||\-|\:)\s*(Buy|Online|Flipkart|Amazon).*$', '', title).strip().upper()
    ...
    emoji = get_emoji(raw_text + " " + title)
    ph_link = explicit_ph or build_price_history_link(canonical, title)
    caption = format_caption(title, coupon, emoji, aff_link, scraped)

    image_url = None
    if msg.get("photo"):
        image_url = get_tg_photo(msg["photo"][-1]["file_id"])
    elif scraped and scraped.get("image_url"):
        image_url = scraped["image_url"]
    elif web_page.get("photo"):
        image_url = get_tg_photo(web_page["photo"][-1]["file_id"])

    post_deal(image_url, caption, ph_link)

def background_bot_loop():
    """Continuous background loop with robust offset management and web server support."""
...
if __name__ == "__main__":
    # Start Flask web server in a separate thread (for Render health check)
    threading.Thread(target=run_web, daemon=True).start()
    
    # Start Telegram bot listener in the main thread
    background_bot_loop()
