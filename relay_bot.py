...
# Amazon-wide cooldown - chahe kitni bhi categories/searches ho, Amazon ko ek saath
# multiple requests na jaayein (isse "rush hour" overload-error trigger hota hai)
_amazon_request_lock = threading.Lock()
_amazon_last_request_time = 0
AMAZON_MIN_GAP_SECONDS = 90

def amazon_throttle():
    """Har Amazon request se pehle call karo - agar koi doosri thread abhi-abhi Amazon
    ko hit kar chuki hai, thoda wait karta hai taaki minimum gap maintain rahe."""
    global _amazon_last_request_time
    with _amazon_request_lock:
        elapsed = time.time() - _amazon_last_request_time
        if elapsed < AMAZON_MIN_GAP_SECONDS:
            wait_time = AMAZON_MIN_GAP_SECONDS - elapsed
            print(f"DEBUG AMAZON-THROTTLE: {wait_time:.1f}s wait kar rahe hain (dusri Amazon request abhi hui thi)")
            time.sleep(wait_time)
        _amazon_last_request_time = time.time()
...
def is_blocked_html(html_content):
    """Check karta hai ki page reCAPTCHA/access-denied/rate-limit/block-page hai ya real content.
    Chhote pages ke liye poore content mein marker dhoondte hain (real content itna chhota
    nahi hota). Bade (real) pages ke liye sirf <title> tag check karte hain - kyunki bade
    legitimate pages mein bhi kahin embedded script/widget mein 'captcha' jaisa word ho
    sakta hai bina asal mein block hue (jaisa Amazon ke hidden security-components mein hota hai)."""
    if not html_content:
        return True
    length = len(html_content)
    markers = ["recaptcha", "are you a human", "access denied", "site maintenance", "captcha",
               "rush hour", "traffic is piling up", "oops!", "service unavailable",
               "automated access", "api-services-support", "developer.amazonservices"]
    ...
    # Bade page ke liye - sirf <title> mein block-marker ho tabhi block maano
...
def fetch_html_via_browser(url, referer=None):
    ...
    try:
        ...
        print(f"DEBUG CATEGORY: Browser-fallback fetch failed: {e}")
        return None, None
    finally:
...
def fetch_category_products_flipkart(category_url, max_products=10, max_pages=4, only_page=None):
    ...
    try:
        ...
        # Agar plain-request se block/reCAPTCHA mila, real Patchright browser se retry karo -
        # JS run hoti hai, real fingerprint hoti hai, isliye behtar chance hai.
        if is_blocked_html(html_content):
            print(f"DEBUG CATEGORY: Blocked via requests, trying browser-fallback...")
            browser_html, browser_final_url = fetch_html_via_browser(base_url, referer="https://www.flipkart.com/")
            if browser_html and not is_blocked_html(browser_html):
                print(f"DEBUG CATEGORY: Browser-fallback SUCCEEDED, HTML length = {len(browser_html)}")
                html_content = browser_html
                base_url = browser_final_url or base_url
            else:
                print(f"DEBUG CATEGORY: Browser-fallback bhi blocked/failed, giving up on this category")
        ...
        for page_num in ([only_page] if only_page else range(1, max_pages + 1)):
            ...
            # Diagnostic - agar 0 mile, alag-alag hisso ko alag se count karo taaki pata chale
            # kaunsa exact part regex se match nahi ho raha
            if len(card_links) == 0:
                pid_count = len(re.findall(r'pid=([A-Z0-9]+)', page_html))
                itm_count = len(re.findall(r'/p/itm[a-zA-Z0-9]+', page_html))
                href_count = page_html.count('href="')
                print(f"DEBUG CATEGORY: Page {page_num} diagnostic - href= count: {href_count}, 'pid=' count: {pid_count}, '/p/itm' count: {itm_count}")
            ...
            for relative_url, itm_id, pid in card_links:
                ...
                window = page_html[link_pos: link_pos + 6000]
...
def fetch_category_products_amazon(category_url, max_products=10, max_pages=3, only_page=None):
    ...
    try:
        ...
        if is_blocked_html(html_content):
            print(f"DEBUG CATEGORY (Amazon): Blocked via requests, trying browser-fallback...")
            browser_html, browser_final_url = fetch_html_via_browser(base_url, referer="https://www.amazon.in/")
            if browser_html and not is_blocked_html(browser_html):
                print(f"DEBUG CATEGORY (Amazon): Browser-fallback SUCCEEDED, HTML length = {len(browser_html)}")
                html_content = browser_html
                base_url = browser_final_url or base_url
            else:
                print(f"DEBUG CATEGORY (Amazon): Browser-fallback bhi blocked/failed, giving up on this category")
...
COLOR_WORDS = {
    ...
    "khaki", "olive", "teal", "wine", "mustard", "peach", "lavender", "multicolor",
    "multi", "coffee", "charcoal", "rust", "mint", "lilac", "magenta", "tan"
}

def normalize_title_for_dedup(title):
    """Title se color-words hata ke ek 'base' version banata hai, taaki same product
    ke alag-alag color-variants ko duplicate detect kiya ja sake (Flipkart har color
    ko alag Product ID deta hai, isliye pid-based dedup inhe alag samajhta hai)."""
    if not title:
        return ""
...
def filter_category_products(products, min_discount=0, min_rating=0, price_min=0, price_max=None, exclude_pids=None):
    """Products ko discount %, rating, price range, already-posted status, aur
    same-product-alag-color duplicate ke hisaab se filter karta hai."""
    ...
    for p in products:
        ...
        base_title = normalize_title_for_dedup(p.get("title"))
        if base_title and base_title in seen_base_titles:
            continue  # same product ka doosra color-variant hai, skip karo
