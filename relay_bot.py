...
def fetch_category_products_flipkart(category_url, max_products=10):
    ...
    try:
        ...
        resp = requests.get(category_url, headers=headers, timeout=15, allow_redirects=True)
        html_content = resp.text

        print(f"DEBUG CATEGORY: Final URL = {resp.url}")
        ...
        # Plain requests JS run nahi karta, isliye us URL ko manually nikaal ke dobara fetch karo.
        js_redirect = re.search(r'cashbackUrl\s*=\s*["\']([^"\']+)["\']', html_content)
        if js_redirect:
            real_url = js_redirect.group(1)
            ...
            html_content = resp.text
            print(f"DEBUG CATEGORY: Final URL (after JS redirect) = {resp.url}")
            print(f"DEBUG CATEGORY: HTML length (after JS redirect) = {len(html_content)}")

        print(f"DEBUG CATEGORY: Sample = {html_content[:3000]}")

        products = []
        seen_pids = set()

        card_links = re.findall(r'href="(/[^"]*?/p/(itm[a-zA-Z0-9]+)\?pid=([A-Z0-9]+)[^"]*)"', html_content)
        print(f"DEBUG CATEGORY: Raw card_links found = {len(card_links)}")
        ...
        for relative_url, itm_id, pid in card_links:
            ...
            link_pos = html_content.find(relative_url)
            window = html_content[link_pos: link_pos + 4000]
            ...
            # Dedicated MRP patterns first (Flipkart listing-card class names + generic strike-through)
            mrp_match = re.search(
            ...
            if mrp_match:
                ...
                # Fallback: agar dedicated MRP class na mile, doosra ₹ number try karo
                try:
...
def fetch_category_products_amazon(category_url, max_products=10):
    ...
    success-rate Flipkart wale se kam ho sakta hai - yeh best-effort hai."""
    try:
        headers = {
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/122.0.0.0 Safari/537.36",
            "Accept-Language": "en-IN,en;q=0.9",
        ...
        print(f"DEBUG CATEGORY (Amazon): Final URL = {resp.url}")
        ...
        # Amazon search-result cards: <div data-component-type="s-search-result" data-asin="XXXXXXXXXX" ...>
        card_blocks = re.findall(r'data-component-type="s-search-result"[^>]*data-asin="([A-Z0-9]{10})"', html_content)
        print(f"DEBUG CATEGORY (Amazon): Raw asin cards found = {len(card_blocks)}")
        ...
        for asin in card_blocks:
            ...
            asin_pos = html_content.find(f'data-asin="{asin}"')
            window = html_content[asin_pos: asin_pos + 5000]
...
def load_posted_pids():
    try:
        with open(CATEGORY_STATE_FILE, "r") as f:
            return set(json.load(f).get("posted_pids", []))
    except:
        return set()
...
def save_posted_pids(pids):
    try:
        with open(CATEGORY_STATE_FILE, "w") as f:
            json.dump({"posted_pids": list(pids)[-1000:]}, f)
    except Exception as e:
